...
The top-level `code-from-spec/_node.md` is the **root node**.
...
Every node has a logical name derived from its position in the
tree, using the `SPEC/` prefix. Two more prefixes complete the
reference vocabulary: `ARTIFACT/` references the artifact
generated by a node, and `EXTERNAL/` references a plain project
file.

A logical name without a parenthetical qualifier is an
**unqualified name** (`SPEC/x/y`). Adding a qualifier makes
it a **qualified name** (`SPEC/x/y(z)`), which targets a
specific public subsection of the node (see Body).
...
name (e.g. `# SPEC/architecture/backend/config`). Its content
