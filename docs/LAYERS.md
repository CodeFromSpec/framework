...
`imports: ARTIFACT/`) and produce artifacts for the next
...
- **`imports: ARTIFACT/`** — a leaf node imports artifact
...
`imports` to consume artifacts generated by leaf nodes in
another subtree.
...
│   ├── _node.md              ← conventions for the functional layer
│   └── transfers/
│       └── _node.md          ← imports: ARTIFACT/extraction/payments-proto
```
...
other nodes reference via `imports: ARTIFACT/`.
