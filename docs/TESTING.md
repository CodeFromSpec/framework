...
How to organize test specs so that generated code is verified
by generated tests — and why the way they are organized decides
whether the verification means anything.
...
under `test/utils/`; the test guard node should require their
use rather than let each test generation reinvent them.

Place test conventions — framework choices, naming patterns,
setup requirements, how expected values are stated — in an
intermediate node at the top of the test subtree, so every
test leaf inherits them. A convention that lives in the test
guard node is applied by every test generation, including
tests written months later.
