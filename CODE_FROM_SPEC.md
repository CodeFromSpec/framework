...
`type: artifact` (see Frontmatter) generate artifacts;
leaf nodes without `type` serve as documentation only.
...
Optional. Declares what the node produces. Recognized
value: `artifact` — the node generates an artifact. When
absent, the node produces nothing: `output`, `imports`,
`input`, and an `# Agent` section are format errors.
