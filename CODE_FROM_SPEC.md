...
Frontmatter is optional YAML metadata in spec nodes. It is not part of the
node's content — it does not participate in inheritance or
`depends_on`. The framework recognizes specific fields (described
below). Unrecognized fields are ignored and may be
used as custom fields by projects.
...
absent, the node does not generate an artifact — it serves
as documentation or as an intermediate node that provides
context to its descendants. Each leaf node generates at most
one artifact.
...
precede it, not even whitespace. The heading is the node's
logical name (e.g. `# SPEC/architecture/backend/config`).
Its content serves as intent — what this node does and why
it exists. This section is not inherited, not importable,
and not included in the chain.
...
generation subagent when generating artifacts for this node. It is not available to
