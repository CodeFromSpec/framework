...
built. A generation subagent receives a linear chain — it has no
concept of nodes, parents, children, or tree position.
Meta-language that leaks tree structure into spec content produces
confusing or incorrect generated artifacts.
...
2. Read each file. For every sentence, apply the core question.
   Focus on `# Public` and `# Agent` sections — those are what
   the generation subagent receives, so meta-language there is
   most harmful. Private sections are only seen by humans and the
   orchestrator, so meta-language there is less critical but
...
the same thing without leaking the abstraction. The version
increment instruction is also framework mechanics — staleness is
now hash-based and automatic.
